...
export const TaskList = ({ tasks, onEditTask, onDeleteTask }: TaskListProps) => {
  ...
  const renderPriorityGroup = (priority: 'P1' | 'P2' | 'P3' | 'P4', tasks: Task[]) => {
    ...
    return (
      <div key={priority} className="mb-8">
        <div className={`flex items-center gap-3 mb-4 p-3 rounded-lg border ${priorityColors[priority]}`}>
          <div className={`w-3 h-3 rounded-full ${
            priority === 'P1' ? 'bg-red-500' :
            priority === 'P2' ? 'bg-orange-500' :
            priority === 'P3' ? 'bg-blue-500' : 'bg-gray-500'
          }`} />
